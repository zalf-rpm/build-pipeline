...
USER_FOLDER=/beegfs/${USER}
# check if user folder exists
if [ ! -d "$USER_FOLDER" ]; then
  # that's wierd, the user folder does not exist, but the user is in the system.
  # maybe the user has no hpc account, but is in the system.
  echo "User folder $USER_FOLDER does not exist." 
  echo "You need a HPC account to run Simplace on the HPC cluster. Please contact the administrator."
  exit 1
fi

# create folders if they do not exist, readable for all users, but writable only for the user
# so our team can access the data for debugging, but the user can not delete the data of other users.
mkdir -p $USER_FOLDER/simplace_simulation
chmod 755 $USER_FOLDER/simplace_simulation

SIMPLACE_WORK=${USER_FOLDER}/simplace_simulation/SIMPLACE_WORK
...
if [ ! -d "$SIMPLACE_WORK" ]; then
  ...
  chmod 755 $SIMPLACE_WORK
  echo "Please copy your work data into $SIMPLACE_WORK."
  exit 1
fi
...
SIMPLACE_OUT=${USER_FOLDER}/simplace_simulation/out
mkdir -p $SIMPLACE_OUT
chmod 755 $SIMPLACE_OUT
SIMPLACE_OUT_ZIP=${USER_FOLDER}/simplace_simulation/output_zip
mkdir -p $SIMPLACE_OUT_ZIP
chmod 755 $SIMPLACE_OUT_ZIP
SIMPLACE_LOG=${USER_FOLDER}/simplace_simulation/log
...
# job name should only contain letters, numbers and underscores, so we replace all other characters with underscores
...
SIMPLACE_OUT=${SIMPLACE_OUT}/${RUN_ID}
mkdir $SIMPLACE_OUT
SIMPLACE_OUT_ZIP=${SIMPLACE_OUT_ZIP}/${RUN_ID}
mkdir $SIMPLACE_OUT_ZIP
SIMPLACE_LOG=${SIMPLACE_LOG}/${RUN_ID}
mkdir $SIMPLACE_LOG
chmod 755 $SIMPLACE_OUT
chmod 755 $SIMPLACE_OUT_ZIP
chmod 755 $SIMPLACE_LOG
...
cd $USER_FOLDER/simplace_simulation
