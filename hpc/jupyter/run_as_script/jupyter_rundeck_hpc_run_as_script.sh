...
OUT_SCRIPT_NAME=${SCRIPT_NAME}_$DATE

# create script for conversion
CONVERSION_SCRIPT=${WORKDIR}/rd_convert_script_${JOB_EXEC_ID}.sh
cat <<EOF > ${CONVERSION_SCRIPT}
#!/bin/bash
source /opt/conda/etc/profile.d/conda.sh
conda activate jupyterenv
jupyter nbconvert --to script "$SCRIPT_NAME" --output $OUT_SCRIPT_NAME
EOF
...
singularity run -H $SINGULARITY_HOME -W $SINGULARITY_HOME --cleanenv \
 ...
 ${IMAGE_PATH} /bin/bash $CONVERSION_SCRIPT

rm -f $CONVERSION_SCRIPT
# get exit status
STATUS=$?
if [ $STATUS != 0 ]; then
echo "Jupyter notebook conversion failed"
exit 1
fi 
...
SCRIPT_INPUT="${MOUNT_PROJECT} ${MOUNT_DATA} ${MOUNT_HOME} ${WORKDIR} ${MOUNT_DATA_SOURCE1} ${MOUNT_DATA_SOURCE2} ${MOUNT_DATA_SOURCE3} ${READ_ONLY_SOURCES} ${JWORK} ${IMAGE_PATH} ${VERSION} ${DATE} ${OUT_SCRIPT_NAME}.py"

echo $CMD_LINE_SLURM
echo $SCRIPT_INPUT

BATCHID=$( sbatch $CMD_LINE_SLURM /beegfs/common/batch/run_script_${VERSION}.sh $SCRIPT_INPUT )
