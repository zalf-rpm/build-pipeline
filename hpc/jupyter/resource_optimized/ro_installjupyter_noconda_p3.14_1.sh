...
if [ ! -e ${ENVPATH} ] ; then
    pipx install jupyter --include-deps --python python3.14
    pipx inject jupyter jupyterlab-git matplotlib pandas ipyparallel jupyterlab-nvdashboard
    pipx ensurepath
fi
...
if [ ! -e $WORKDIR/.envs/python3.14 ] ; then
    echo "Creating python3.14 kernel"

    python3.14 -m venv $WORKDIR/.envs/python3.14
    source $WORKDIR/.envs/python3.14/bin/activate
    pip install ipykernel matplotlib seaborn numpy pandas scipy ipyparallel
    python3.14 -m ipykernel install --user --name python3.14 --display-name "Python 3.14"
    deactivate
fi
...
if [[ $SETUP_AI_ENV_1 == "true" && ! -e $WORKDIR/.envs/pytorchcu121 ]] ; then
    ...
    python3.14 -m venv $WORKDIR/.envs/pytorchcu121
    ...
    python3.14 -m ipykernel install --user --name pytorchcu121 --display-name "PyTorch CUDA 12.1"
    deactivate
fi
