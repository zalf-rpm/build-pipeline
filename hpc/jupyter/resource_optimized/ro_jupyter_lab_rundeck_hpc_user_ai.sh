...
EOF
   ...
   # allocate resources, get a free port and start jupyter lab in a singularity container on the allocated resources
   
   #old
   #CMD_LINE_SLURM="--parsable --job-name=${SBATCH_JOB_NAME} ${HPC_PARTITION} --time=${TIME} -N 1 -c ${CORES} ${MEM_PER_CPU} -o ${LOGS}/jupyter_lab_${DATE}_%j.log"
   ...
   # salloc required resources
   JOBID=$( salloc --job-name=${SBATCH_JOB_NAME} ${HPC_PARTITION} --time=${TIME} -N 1 -c ${CORES} ${MEM_PER_CPU} --immediate 10 --no-shell)
   #TODO: change output so salloc does not print anything if resources cannot be allocated, currently it prints an error
   # should only print jobid if successful, otherwise empty
   sleep 10 # wait a bit to make sure jobid is printed
   wait $JOBID # this will not work
   if [ $? -ne 0 ]; then
      echo "Error: No free slot available. Try again later."
      exit 1
   fi

   # run a job to get a free port
   FREE_PORTS=$(srun --jobid=${JOBID} python -c 'import socket; s=socket.socket(); s.bind(("", 0)); print(s.getsockname()[1]); s.close()')
   JUPYTER_PORT=$FREE_PORTS
   ...
   SCRIPT_INPUT="${USER} ${LOGIN_HOST} ${MOUNT_PROJECT} ${MOUNT_DATA} ${MOUNT_HOME} ${WORKDIR} ${MOUNT_DATA_SOURCE1} ${MOUNT_DATA_SOURCE2} ${MOUNT_DATA_SOURCE3} ${JWORK} ${IMAGE_PATH} ${VERSION} ${JUPYTER_PORT} ${READ_ONLY_SOURCES}"
   
   # start jupyter lab on the allocated resources
   sbatch --jobid=${JOBID} -o ${LOG_NAME} /beegfs/common/batch/jupyter-lab_${VERSION}.sh $SCRIPT_INPUT
   ...
   if [ -f ${LOG_NAME} ] ; then
      #cat ${LOG_NAME}
      echo "Job started"
      NODEHOST=$(squeue -j ${JOBID} --noheader --format="%R" )   
   else 
