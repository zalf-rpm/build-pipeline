...
if [ ! -e $WORKDIR/.envs/python3.12 ] ; then
    echo "Creating python3.12 kernel"
...
if [ ! -e $WORKDIR/.envs/python3.10_default ] ; then

    python3.10 -m venv $WORKDIR/.envs/python3.10_default 
    source $WORKDIR/.envs/python3.10_default/bin/activate
    pip install ipykernel matplotlib seaborn numpy pandas scipy ipyparallel
    python3.10 -m ipykernel install --user --name python3.10_default --display-name "Python 3.10 Default"
    deactivate
...
if [[ $SETUP_AI_ENV_1 == "true" && ! -e $WORKDIR/.envs/pytorchcu121 ]] ; then
    ...
    python3.10 -m venv $WORKDIR/.envs/pytorchcu121
    ...
    python3.10 -m ipykernel install --user --name pytorchcu121 --display-name "PyTorch CUDA 12.1"
    deactivate
fi
